#Author: Bahadır Bozdağ

import datetime

while True:
    birthday = input('Cocugunuzun hangi yılda dogdugunu GG AA YYYY şeklinde giriniz: ')
    ...
    control = True #true false degiskeni yaptim cunku for dongusunu break ile kapattıgımda kodlara devam ediyor
    ...
    for i in birthday: #birthday inputunu döngüye aliyor
        if (i not in numbers): #yazili olan tarihte numbersin icindeki yazilanlardan farkli bir sey varmi diye kontrol ediyor.
            print("Cocugunuzun dogum tarihi sadece sayı içerebilir.")
    ...
    d_birthday = datetime.datetime(int(year1), int(mounth1), int(day1)) #cocugun dogum tarihini datetime modulune kayıt ettirdik.
    ...
    if (l_day.days >= 0 and l_day.days < 30):
        print("Cocugunuzun Hep-B I aşısı olması gerekmektedir.")
        continue
    elif (l_day.days >= 30 and l_day.days < 60):
        print("Cocugunuzun Hep-B II aşısı olması gerekmektedir.")
        continue
    elif (l_day.days >= 60 and l_day.days < 120):
        print("Cocugunuzun BCG I, KPA I ve DaBT-IPA-Hib I aşılarını olması gerekmektedir.")
        continue
    elif (l_day.days >= 120 and l_day.days < 180):
        print("Cocugunuzun KPA II ve DaBT-IPA-Hib II aşılarını olması gerekmektedir.")
        continue
    elif (l_day.days >= 180 and l_day.days < 360):
        print("Cocugunuzun Hep-B III, DaBT-IPA-Hib III ve OPA I aşılarını olması gerekmektedir.")
        continue
    ...
    if (l_day.days >= wf_day.days and l_day.days >= 360 and l_day.days < 540):
        print("Cocugunuzun KPA R, Sucicegi I ve KKK I aşılarını olması gerekmektedir.")
        continue
    elif (l_day.days >= 360 and l_day.days < 540):
        print("Cocugunuzun KPA R ve KKK I aşılarını olması gerekmektedir.")
        continue
    ...
    if (l_day.days <= ha_day.days and l_day.days >= 540 and l_day.days < 720):
        print("Cocugunuzun DaBT-IPA-Hib R, OPA II ve Hep-A I aşılarını olması gerekmektedir.")
        continue
    elif (l_day.days >= 540 and l_day.days < 720):
        print("Cocugunuzun DaBT-IPA-Hib R ve OPA II aşılarını olması gerekmektedir.")
        continue      
    elif (l_day.days <= ha_day.days and l_day.days >= 720 and l_day.days < 1440):
        print("Cocugunuzun Hep-A II aşısı olması gerekmektedir.")
        continue
    ...
    #KKK ve DaBT-IPA R asisi icin ve 13 yasinda vurulması icin kontrol koydurduk.
    if (l_day.days <= da_day.days and l_day.days >= 1440 and da_year < 13):
        print("Cocugunuzun KKK ve DaBT-IPA R aşısı olması gerekmektedir.")
        continue
    elif (da_year == 13):
        print("Cocugunuzun Td R aşısı olması gerekmektedir.")
        continue
